...
def adaptive_quota(
        ...
        crisis_rate: float = 1.1616014660103775, #1.0,
    ...
    if crisis_level <= -1:
        ...
        # compute available water to share
        total_demand = (avg_pump * (actors_priority > 0)).sum()
        available_water = total_demand * crisis_rate / max(1, crisis_level)
        ...
        scale = 1 / np.power(scale, 0.25) # 0.25 could be learnt too
...
def hard_ecofair_incentives_policy(
    ...
    crisis_sensitivity = 4.887128330883843, #2.0,
    equity_factor = 26.092559856664767, #10.0,
    alpha = 0.49724850589238545, #0.6375574713552131, #0.0,
    ...
    # get crisis level from last river flow
    crisis_level = is_crisis[-1]

    # crisis_level is computed with last river flow and if that led to a crisis
    # we also check if all current demands are met, would there be a crisis. i.e 
    # i.e., if all actors took their quota - would the river flow result in a crisis
    # this helps to regulate alertness to ecological stress

    # COMPUTE CURRENT ECOLOGICAL CRISIS!
    # get water predictions, total_demand to estimate remaining water
    # current water flow and actor's demand
    estimated_remaining_water = water_flows[-1] - avg_pump.sum()
    ...
    crisis_level = alpha * next_crisis + (1 - alpha) * crisis_level

    # check for ongoing crisis
    if len(is_crisis) > 1:
        if crisis_level >= 1 and is_crisis[-2] >= 1 :
            crisis_level += is_crisis[-2]
    ...
    penalty_spread = equity_factor * np.exp(-crisis_sensitivity * c)
...
# UNSTABLE DURING OPTIMIZATION DUE TO EXP EXPLODING
...
def smooth_ecofair_incentives_policy(
    ...
    c = hard_crisis_level + 1
    overuse_coef = np.exp(-crisis_sensitivity * c)
